...
Every user story carries an **Independent Test** — keep it *measurable*: a clear pass/fail the agent
can check itself (e.g. "uploading a CSV with 3 rows creates 3 records and shows a success message"),
...
build, run `powershell -File scripts\check-plan.ps1 -Gate` — it FAILS if any task is still unchecked
or any test is too vague to verify. The script, not the agent's word, decides the build matched the plan.
...
- Each story's Independent Test must be objective pass/fail, so the agent can self-verify and run longer
