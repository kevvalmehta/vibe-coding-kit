...
> **Routing note (vibe-coding-kit):** `/design-craft` cites **§0 (design read), §1 (dials) and
> §4 (bias correction)** — read only the section the step names. **§2–§3 name a specific stack
> (React/Next/Tailwind); they yield to whatever `/stack` chose for this project.** §5's animation
> skeletons are superseded by `references/premium-motion.md` + `references/layout-skeletons.md`,
> and §9's tells list by `references/tells.md` — prefer those files; the sections remain here
> unmodified for provenance (vendored MIT file).
