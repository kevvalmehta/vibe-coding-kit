...
| "Output quality is a coin flip" / "make this actually good" / same correction keeps recurring | `/quality-charter` | Installs the quality system: numbered house rules with the failure that motivated each, script gates for what's mechanically checkable, a scored self-review for judgment calls, exemplar banks, and a learnings loop that turns repeat corrections into permanent checks. Writes quality-system files; never pushes/merges/deploys |
| Creating a new skill or plugin, or fixing one that gets ignored/half-followed | `/skill-craft` | The skill-writing coach: one job per skill, reliable trigger descriptions, short checkable bodies, five failure modes, registration. Routes to skill-creator (Anthropic plugin, if installed) + `/quality-charter` |
| Building anything with a screen / "it looks AI-generated or generic" | `/design-craft` | The design coach: a deliberate look decided BEFORE markup (binding `DESIGN-SPEC.md` — palette, real fonts, grid, named layout skeletons, motion budget), real craft while building, then a deterministic tell scan (`scripts/devibe_scan.py`) + a scored self-critique (≥12/14 to present). Bans undecided defaults, never the owner's choices |
| "What tables do I need?" / the app must remember things | `/data-model` | The database coach: plain-English entity interview → binding `DATA-MODEL.md` contract approved before any table; additive-first migrations; RLS sentence on every user-owned table |
| "Add login" / user accounts / "protect this page" | `/add-login` | The login coach: magic-links default, `AUTH-SPEC.md` contract, managed auth only, RLS everywhere, mandatory wrong-user test. Routes to `/data-model` if tables aren't defined |
| "Take payments / send email / file uploads" | `/wire-up` | The integrations coach: boring defaults (Stripe/transactional email/stack storage), `INTEGRATIONS.md` contract, test mode first, webhook signatures mandatory, owner flips live keys |
| Just launched — "how do I know if it goes down?" | `/uptime` | Free outside checker + `UPTIME.md` watch list + incident runbook; proves the alert fires before claiming setup. Reachability only |
| "How much will this cost me?" / "set a spending limit" | `/cost-watch` | Inventories billable services → `COSTS.md` → owner-at-keyboard cap-or-alert on each (AI keys get usage limits) + a monthly 5-minute review |
