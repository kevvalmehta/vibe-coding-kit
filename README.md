...
| `codebase-design` | `codebase-design` — shared vocabulary for designing DEEP MODULES (a lot of behaviour behind a small interface): Module / Interface / Implementation / Adapter / Depth / Seam + five principles (make modules deep, put the seam where the interface is simplest, design for the caller, pull complexity down, deep = testable). Design-aid pulled by `/speckit-plan` / `grill-with-docs` / `/audit`; distinct from `/design-craft` (UI) + `/data-model` (tables) + domain-modeling (business terms). Never edits code — real changes route to `/safe-change`. Adapted from mattpocock/skills (MIT) + Ousterhout |
| `loop-design` | `/loop-design` — recommends the execution shape (one-shot / repeat-until-verified loop / subagent fan-out / multi-step workflow); for a loop fills goal · success measure · next-step rule · STOP condition (+`catalog.md`: proven loop patterns from the MIT loop-library). Fires during `/speckit-plan`; recommendation-only. Read-only |
| `upstream-check` | `/upstream-check` — watches the repos this kit borrowed from (listed in `upstream-sources.md`), reports what each changed since the last check with pros/cons + a worth-it verdict, you approve each port (built via `/safe-change`). On-demand only; never auto-copies, pushes, or deploys. Read-only |
