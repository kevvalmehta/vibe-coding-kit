...
| `recall.py` | Search your saved memory AND past session transcripts by keyword (`python scripts/recall.py <words>`; `--all` = every project) — the recall half of the memory fix. Claude Code already stores every session in full; this greps them + your memory files and prints dated matches, so an old decision surfaces instead of feeling lost. Local-only: no network, no AI, no telemetry |
